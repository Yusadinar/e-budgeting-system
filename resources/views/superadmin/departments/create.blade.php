...
<div class="max-w-lg animate-page">
    ...
    <div class="bg-white rounded-2xl border border-slate-100 p-6 shadow-card">
        <h3 class="text-lg font-semibold text-slate-800 mb-6">Informasi Departemen Baru</h3>

        <form method="POST" action="{{ route('superadmin.departments.store') }}" class="space-y-5">
            @csrf

            <div>
                <label for="dept_name" class="block text-sm font-medium text-slate-700 mb-1.5">Nama Departemen</label>
                <input type="text" name="dept_name" id="dept_name" value="{{ old('dept_name') }}" required
            ...
            <div>
                ...
                <input type="text" name="budget_code" id="budget_code" value="{{ old('budget_code') }}" required
